...
function Certificates() {
  ...
  return (
    <div id="certificates"
      ...
      <h1 className={`${pageStyles.section_heading} text-4xl font-bold`}>
        <span className={pageStyles.numbering}>05.</span> Certifications
      </h1>
        ...
        <div id="panel"
            ...
            <div className="relative w-full flex items-center justify-center">
              {activeCertificate && (
                <Image
                  ...
                  width={activeCertificate.width} // Use width from data
                  height={activeCertificate.height} // Use height from data
                  className="max-w-full max-h-96 h-auto rounded-lg shadow-lg object-contain"
                />
              )}
            </div>
            <div className="flex justify-between w-full mt-4">
              <button onClick={handlePrevCertificate}
                className={styles.img_button}>
                Previous
              </button>
              <button onClick={handleNextCertificate}
                className="font-bold p-2 rounded-lg bg-cyan-500 hover:bg-cyan-600 text-white transition-transform transform hover:scale-105">
                Next
              </button>
            </div>
          </div>
          <div className="flex w-full overflow-auto scrollbar scrollbar-track-blue-400 scrollbar-thumb-blue-800">
            {tabs.map((tab, index) => (
              <div
                key={index}
                className={`p-4 border-x border-t-2 md:p-5 lg:p-6 ${styles.panel_item} text-nowrap text-center cursor-pointer transition-colors duration-300 ${activeTab === tab.name ? 'bg-cyan-500 text-white' : 'hover:bg-cyan-100 text-gray-700'}`}
